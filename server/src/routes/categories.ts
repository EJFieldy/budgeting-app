...
router.put("/budgets/:id", async (req, res, next) => {
    try {
        ...
        const budgetData = req.body.monthlyBudget;

        if (!budgetData) {
            return res
                .status(400)
                .json({ error: "Amount for monthly budget required" });
        }

        const updatedBudget = await prisma.category.update({
            where: {
                id,
            },
            data: {
                monthlyBudget: parseFloat(budgetData),
            },
        });

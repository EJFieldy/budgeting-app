...
A limitation of the web app in its current state is that it is designed for personal use by a single individual. Adding a user model to the database schema and creating a one to many relationship between users and transactions via foreign keys, would begin to move it away from being a personal use project towards a production-ready application. For the correct financial data to be shown to the associated user, login information will need to be stored in the user model in the form of a username and password. The username could be in the form of an email, or a simple string where the user chooses their own username. Whatever form the username takes, it must be unique to ensure correct identification. To add further security, passwords must be hashed and stored in this format.

With user accounts in place, secure authentication methods are essential to prevent information theft. Whilst the app is not designed to be linked to an individual's bank account, nor should it store any sensitive banking information, it is still important that the security of any financial data stored be a priority, to ensure a user's peace of mind and confidence in the application. To implement this feature, `JSON Web Tokens (JWT)` could be used to handle authorisation. A login route can be created to handle accepting user credentials, verifying their validity and generating a JWT token to be returned and stored in an HTTP-only cookie. A logout route will handle ending the session and invalidating the JWT token. The client side login state can be handled by React's Context API and will update after login and logout API calls. CSRF protection will need to be included to mitigate the security risks associated with cookie based storage. Finally, a user route can be made which will incorporate middleware in charge of checking whether a user is authenticated or not. If they are, they are allowed access to protected routes, such as accessing the dashboard.

### Data Visualisation and Graphs
In its current form, the app features a pie chart on the dashboard displaying total expenses by category. This is helpful information, but is lacking in depth and could be taken further with the inclusion of graphs for data visualisation on pages such as the transactions and budgets pages. For example, a line chart could be placed on the transactions page showing a user's spending over time, with expenses on the y-axis and time in months on the x-axis. A `/summary` endpoint already exists which creates date values for the start of the month and the start of next month, and uses a Prisma `findMany` request to the `Categories` model including associated transactions between these two dates. This provides the base structure to create this feature, however it would need to be extended. A new endpoint which takes this monthly aggregate data and returns a new object containing keys pointing to each month, as well as a sum of total expenses across all categories each month, would also need to be made. 

I used the `recharts` library to create the pie chart seen on the dashboard, and it could be reused here to create the line graph discussed. The `<LineChart />` component with the aggregate monthly categories data from the newly created endpoint as its data source would be suitable for this feature. The `<Tooltip />` could be customised to display further information to the user, such as a percentage change from the previous month, a percentage of that month compared to the total expenses across all months, and which category had the largest expense total. This sort of graph would give a clearer insight into the user's spending habits, and allow them to make informed decisions moving forward.

### Time Period Filter
The dashboard, transactions and budgets pages are capable of showing monthly aggregate data, but only for the current month. Whilst suitable for the dashboard, it is an inconvenience for the other pages. A filtering feature, allowing the user to specify a start and end date, would enable the app to display historical financial data. Currently, the `TransactionsOverview` page receives information from the `Transaction` model, ordered by date from most recent to oldest. The same is true for the `Budgets` page, but from the `Category` model instead. Two new endpoints would need to be created, which could extract two values from `req.query`, create two date variables, and make a Prisma `findMany` request for information between these two parameters. For the `TransactionsOverview` page this information is enough for the display, however the `Budgets` page requires aggregate data to display correctly. Therefore, the budget API endpoint requires an extra step calculating aggregate monthly data between these two date variables before returning the new data object. These calculations have already been done in the `/summary` endpoint in the `categories` router, so would be simple to replicate.

The frontend needs to facilitate the submission of these dates. One possibility is to use the `<DateRangePicker />` component from the `React Aria` library wrapped in a form with a custom `handleSubmit` event handler, to submit the `startDate` and `endDate` as query parameters. The component provides the `maxValue` and `isDateUnavailable` props which can be passed for validation before submission. `maxValue` can set the most recent selectable date to today's date, and `isDateUnavailable` can disable dates that have no historical data associated with them. Validating entries in this manner is important to prevent unexpected behaviour and errors occurring from searching for data that doesn't exist.

### Other Considerations
Other possible features include recurrent payments, allowing users to set up monthly debits to be automatically added as expenses. Individual users may specify a date for each recurrent payment. Furthermore, search functionality could be implemented to allow users to find transactions by keyword. This keyword could be a category or perhaps wording used in the optional description associated with each transaction. The final feature to mention is one which enables exporting transaction balance sheets. Users may wish to export their transaction data from the app for storage with other financial data, such as their investment portfolio and their pension. These features, alongside the three features detailed above, would add significant improvements to the application's functionality and quality of life for the user.
